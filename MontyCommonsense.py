...
__version__="2.0"
import zlib
from MontyLingua import MontyUtils,MontyLemmatiser,MontyTagger
...
class MontyCommonsense:
    ...
    def __init__(self,MontyLemmatiser_handle=None,MontyTagger_handle=None):
        ...
        cat_p=self.setitem
        # breakpoint()  # random guess at latin-1 encoding worked...
        list(map(lambda stripped:cat_p(ps1,stripped[0],(stripped[1].split(),stripped[2].split())),
            [tmps.split('|') for tmps in [tagged_cleaned for tagged_cleaned in zlib.decompressobj().decompress(hash1.read()).decode("latin-1").split('\n') if tagged_cleaned.strip()!='']]))
        self.cssdb=ps1
    ...
    def cs_verify_tagged(self,tagged):
        ...
        table_arr=[tagged_cleaned.split('/')[1] for tagged_cleaned in cmp1]

        if len([tagged_cleaned for tagged_cleaned in table_arr if tagged_cleaned in hostnames_dict])>0:
            return tagged
        groupnames_cleaned=' '.join([tagged_cleaned.split('/')[0] for tagged_cleaned in cmp1])
            ...
            for buffer_dict in table_arr:
                ...
                if buffer_dict in hostnames_dict:
                    ...
                    if csplit_cleaned:
                        ...
                        break
        info1=' '.join([tagged_cleaned[0]+'/'+tagged_cleaned[1] for tagged_cleaned in [tmps.split('/') for tmps in cmp1]])
    ...
    def unpp(self,pp):
        ...
        hash1,input_p=[int(tagged_cleaned.split('=')[1]) for tagged_cleaned in chgrp_p.pop().split(';')[:2]]
    ...
    def build_cs_selection_db(self):
        ...
        for buf_cleaned in list(ps1.keys()):
            buffer_arr,ress=ps1[buf_cleaned]
            groups_cleaned,built_in_arr={},{}
            list(map(lambda tagged_cleaned:cat_p(groups_cleaned,tagged_cleaned,1),buffer_arr))
            list(map(lambda tagged_cleaned:cat_p(built_in_arr,tagged_cleaned,1),ress))
            buffer_arr=list(groups_cleaned.keys())
            ress=list(built_in_arr.keys())
            ps1[buf_cleaned]=(buffer_arr,ress)
        cp_dict=open(self.cssdb_filename,'wb')
        buf_arr=[]

        for more in list(ps1.items()):
