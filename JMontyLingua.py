...
class JMontyLingua(java.lang.Object):
    ...
    def jist_predicates(self,text):
        ...
        return '\n\n'.join(['\n'.join(x) for x in svoos_list])

    def tag_text(self,text):
        "@sig public String tag_text(String text)"
        sentences = self.theMontyLingua.split_sentences(text)
        tokenized = list(map(self.theMontyLingua.tokenize,sentences))
        tagged = list(map(self.theMontyLingua.tag_tokenized,tokenized))
        return '\n\n'.join(tagged)
    ...
    def chunk_text(self,text):
        ...
        tokenized = list(map(self.theMontyLingua.tokenize,sentences))
        tagged = list(map(self.theMontyLingua.tag_tokenized,tokenized))
        chunked = list(map(self.theMontyLingua.chunk_tagged,tagged))
        return '\n\n'.join(chunked)
    ...
    def lemmatise_text(self,text):
        ...
        tokenized = list(map(self.theMontyLingua.tokenize,sentences))
        tagged = list(map(self.theMontyLingua.tag_tokenized,tokenized))
        lemmatised = list(map(self.theMontyLingua.lemmatise_tagged,tagged))
        return '\n\n'.join(lemmatised)
