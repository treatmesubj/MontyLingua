...
__version__="2.0"
from MontyLingua import MontyLexiconCustom
import sys
...
class MontyLexiconFast:
    ...
    def load_fastlexicon(self):
        ...
        res_arr=open(c_p,'r')
        built_in_p,input_arr,chgrp1,cd_arr=[int(hostnames) for hostnames in res_arr.read().split()]
        res_arr.close()
    ...
    def get(self,word,default):
        ...
        while alias_p>=alias1 and alias1<chgrp_str:
            cron_cleanedo=int((alias_p-alias1)/2+alias1)
            # breakpoint()
    ...
    def populate_lexicon_from_file(self,filename):
        ...
        print(the_parser_dict)
        cleaned_p=''.join([hostnames[0] for hostnames in line_str])
        popd_dict=''.join([hostnames[1] for hostnames in line_str])
