...
('restring','restring','')
    ...
    def __init__(self):
        ...
        self.regular_any,self.regular_verb,self.regular_noun,self.irregular_re_any,self.irregular_re_verbs,self.irregular_re_nouns=[[[re.compile('^'+the_tokenizer_str[0].lower()+'$')]+the_tokenizer_str[1:] for the_tokenizer_str in the_tokenizers] for the_tokenizers in filename_str]
        self.exceptions_db={}
        ...
        cd_cleaned=[the_tokenizer_str.split() for the_tokenizer_str in pathname_str.read().split('\n')]
        pathname_str.close()
        list(map(lambda the_tokenizer_str:c(hostname_cleaned,the_tokenizer_str[0]+'/'+the_tokenizer_str[1],the_tokenizer_str[2]),cd_cleaned))
        print("Lemmatiser OK!")
        return

    def lemmatise_untagged_sentence(self,untagged):
        the_parser_dict=' '.join([the_tokenizer_str+'/UNK' for the_tokenizer_str in untagged.split()])
        return self.lemmatise_tagged_sentence(the_parser_dict)

    def lemmatise_tagged_sentence(self,tagged):
        cmp=self.lemmatise_word
        domain_str=[the_tokenizer_str.split('/') for the_tokenizer_str in tagged.split()]
        ...
        domain_str=['/'.join(the_tokenizer_str) for the_tokenizer_str in domain_str]
        cal=' '.join(domain_str)
        return cal
    ...
    def verify_lemmatiser(self):
        ...
        pathname_str.write('\n'.join([' '.join(the_tokenizer_str) for the_tokenizer_str in arg_cleaned]))
        pathname_str.close()
        return arg_cleaned
    ...
    def make_verification_dictionary(self):
        ...
        factor=output.split('\n')
        factor=[the_tokenizer_str for the_tokenizer_str in factor if the_tokenizer_str[:3]!=';;;']
        factor=[the_tokenizer_str.split() for the_tokenizer_str in factor]
        factor=[the_tokenizer_str for the_tokenizer_str in factor if len(the_tokenizer_str)>=3]
        factor=[[the_tokenizer_str[0],the_tokenizer_str[1],the_tokenizer_str[2]] for the_tokenizer_str in factor]
        factor=[the_tokenizer_str for the_tokenizer_str in factor if the_tokenizer_str[1][-3:]!='ize' and the_tokenizer_str[1][-7:]!='ization' and the_tokenizer_str[1][4:]!='izer' and '-' not in the_tokenizer_str[1]]
        return factor
    ...
    def check_dictionary_exceptions(self,word,pos=""):
        ...
        if pos=="verb":
            built_in_cleaned=[the_tokenizer_str[0] for the_tokenizer_str in self.irregular_verbs]
            ...
            pairs_dict=[the_tokenizer_str[0] for the_tokenizer_str in self.irregular_nouns]
...
if __name__=='__main__':
    ...
    print([p.lemmatise_word(the_tokenizer_str,) for the_tokenizer_str in test_string.split()])