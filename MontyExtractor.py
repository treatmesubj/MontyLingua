...
class MontyExtractor:
    ...
    def extract_info(self,chunked_text,lemmatise_function_handle=None):
        ...
        the_parser_cleaned=[output_p[0] for output_p in [alias_str for alias_str in id_p if alias_str[1]=='AX']]
        groupnames_p=[output_p[0] for output_p in [alias_str for alias_str in id_p if alias_str[1]=='NX']]
        ...
        j_str=[output_p[0] for output_p in [alias_str for alias_str in id_p if alias_str[1]=='VX']]
        groups_arr=[' '.join(alias_str) for alias_str in factor1(chunked_text)]
        print(chunked_text)
        res_arr=self.extract_pos(chunked_text,['JJ','JJS','JJR','RB','RBR','RBS'])
        cmp_str=self.find_verb_arg_structures(chunked_text)
        cleaned1=[pathname_arr(alias_str,enabled_dict) for alias_str in cmp_str]
        dirname1=[input_str(alias_str,enabled_dict) for alias_str in self.find_verb_arg_structures(chunked_text)]
        dict['noun_phrases_tagged']=groupnames_p
        dict['noun_phrases']=list(map(cp_cleaned,groupnames_p))
        dict['verb_phrases_tagged']=j_str
        dict['verb_phrases']=list(map(cp_cleaned,j_str))
        dict['adj_phrases_tagged']=the_parser_cleaned
        dict['adj_phrases']=list(map(cp_cleaned,the_parser_cleaned))
        dict['prep_phrases_tagged']=groups_arr
        dict['prep_phrases']=list(map(cp_cleaned,groups_arr))
        dict['modifiers_tagged']=res_arr
        dict['modifiers']=list(map(cp_cleaned,res_arr))
    ...
    def make_parameterized_predicates(self,verbose_verb_arg_structure,lemmatise_function_handle=None):
        ...
        cal=[]
        inputs=[]
        user_cleanedh=[[] for alias_str in output_pu]
        alias1=[output_p.split('/')[0] for output_p in [alias_str for alias_str in names_arr.split() if alias_str.split('/')[1]in['IN','TO']]]

        if len(alias1)>0:
            inputs.append('prep='+alias1[0])
        cksum_p=[output_p.split('/')[0] for output_p in [alias_str for alias_str in names_arr.split() if alias_str.split('/')[1]in['DT','CD','PRP$']]]
        ...
        for cp_arr in range(len(output_pu)):
            alias1=[output_p.split('/')[0] for output_p in [alias_str for alias_str in output_pu[cp_arr].split() if alias_str.split('/')[1]in['IN','TO']]]

            if len(alias1)>0:
                user_cleanedh[cp_arr].append('prep='+alias1[0])
            cksum_p=[output_p.split('/')[0] for output_p in [alias_str for alias_str in output_pu[cp_arr].split() if alias_str.split('/')[1]in['DT','CD','PRP$']]]
        ...
        if 'not/rb' in [alias_str.lower() for alias_str in cron_cleaned.split()]:
            cal.append('negation')
        ...
        if lemmatise_function_handle==None:
            ...
            output_pu=list(map(cp_cleaned,output_pu))
        else :
            table_arr=chroot
            a=names_arr
            hostnamessx=[alias_str for alias_str in output_pu]
            chroot=lemmatise_function_handle(chroot)
            names_arr=lemmatise_function_handle(names_arr)
            output_pu=list(map(lemmatise_function_handle,output_pu))
            chroot=hostnamess(chroot)
            names_arr=hostnamess(names_arr)
            output_pu=list(map(hostnamess,output_pu))
            ...
            if _hugo_p:
                ...
                for cron in [alias_str.lower().split('/')[0] for alias_str in cron_cleaned.split()]:
    ...
    def make_concise_verb_arg_structure(self,verbose_verb_arg_structure,lemmatise_function_handle=None):
        ...
        if lemmatise_function_handle==None:
            ...
            names_arr=hostnamess(names_arr)
            output_pu=list(map(hostnamess,output_pu))
        tmp1='("'+chroot+'" "'+names_arr+'" '+' '.join(['"'+alias_str+'"' for alias_str in output_pu])+')'
        return tmp1

    def jist_verb_chunk(self,verbchunk):
        env_cleaned=verbchunk.split()
        env_cleaned=[alias_str for alias_str in env_cleaned if alias_str not in['(VX','VX)']]
        env_cleaned=[alias_str.split('/') for alias_str in env_cleaned]
        env_cleaned=[alias_str for alias_str in env_cleaned if alias_str[0]=='not' or alias_str[1]not in['MD','RB','TO']]
        case_cleaned=list(range(len(env_cleaned)))
        ...
        for cp_arr in case_cleaned:
            ...
            if env_cleaned[cp_arr][1]in['VB','VBD','VBG','VBN','VBP','VBZ']:
                inputsa=1
                continue
        env_cleaned=[alias_str for alias_str in env_cleaned if alias_str[1]!='DELETE']
        env_cleaned=[alias_str[0]+'/'+alias_str[1] for alias_str in env_cleaned]
    ...
    def _find_linked_subject(self,toks,vc_start_index):
        case_cleaned=list(range(0,vc_start_index))
        case_cleaned.reverse()
        names_arr=[]
    ...
    def _find_linked_objects(self,toks,vc_end_index,prev_obj=''):
        output_pu=[]
        iter=list(range(vc_end_index+1,len(toks)))
        ...
        output_pu=[alias_str for alias_str in output_pu if alias_str!='']
        return output_pu
    ...
    def extract_pos(self,tagged_text,pos_whitelist,white_or_blacklist='white'):
        ...
        env_cleaned=[alias_str for alias_str in env_cleaned if alias_str not in['(NX','NX)','(VX','VX)']]
        env_cleaned=[alias_str.split('/') for alias_str in env_cleaned]
        dirname_arr=pos_whitelist

        if white_or_blacklist.lower()=='black':
            env_cleaned=[alias_str for alias_str in env_cleaned if alias_str[1]not in dirname_arr]
        else :
            env_cleaned=[alias_str for alias_str in env_cleaned if alias_str[1]in dirname_arr]
        env_cleaned=['/'.join(alias_str) for alias_str in env_cleaned]
        return env_cleaned
    ...
    def extract_phrases(self,chunked_text,only_salient_words_p=1):
        ...
        for more in range(len(the_tokenizer1)):
            ...
            hostname_arrd=[alias_str.split('/') for alias_str in hostname_cleaned[0].split()]
            hostname_arrd=[alias_str for alias_str in hostname_arrd if alias_str[1]not in['DT','MD']]
            hostname_arrd=[alias_str for alias_str in hostname_arrd if alias_str[0].lower()not in table_str]
            env_str=[alias_str[0] for alias_str in hostname_arrd]
            popd=' '.join(env_str).lower().strip()

            if popd=='':
                continue
            hostname_arrd=[alias_str[0]+'/'+alias_str[1] for alias_str in hostname_arrd]
    ...
    def filter_by_tag(self,chunked_text,blacklist):
        ...
        for cp_arr in range(len(env_cleaned)):
            ...
            if env_cleaned[cp_arr].split('/')[1]in blacklist:
                env_cleaned[cp_arr]=''
        env_cleaned=[alias_str for alias_str in env_cleaned if alias_str!='']
        return ' '.join(env_cleaned)

    def strip_tags(self,chunked_text):
        env_cleaned=chunked_text.split()
        env_cleaned=[alias_str for alias_str in env_cleaned if '/' in alias_str]
        env_cleaned=[alias_str.split('/')[0] for alias_str in env_cleaned]
        return ' '.join(env_cleaned)

    def strip_tags_lemmatised(self,lemmatised_text):
        env_cleaned=lemmatised_text.split()
        env_cleaned=[alias_str for alias_str in env_cleaned if '/' in alias_str]
        env_cleaned=[alias_str.split('/')[2] for alias_str in env_cleaned]
        return ' '.join(env_cleaned)