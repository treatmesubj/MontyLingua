...
__version__="2.0"
import zlib
from MontyLingua import MontyUtils
...
class MontyNLGenerator:
    ...
    def __init__(self):
        ...
        # breakpoint()  # random guess at latin-1 encoding
        cats=zlib.decompress(case_p.read()).decode("latin-1").split('\n')

        for names in cats:
            output,domain_arr=names.split('=')
            domain_arr=tuple([tuple(arg_cleaned.split(',')) for arg_cleaned in domain_arr.split(';')])
            domain_arr=[(arg_cleaned[0],hostnames(arg_cleaned[1])) for arg_cleaned in domain_arr]
            alias_cleaned[output]=domain_arr

    def generate_summary(self,vsoos):
        info_p=list(map(self.generate_sentence,vsoos))
        groups_dict=''
        buf_cleaned=0
    ...
    def get_features(self,feature_string):
        ...
        if feature_string not in tmp_str:
    ...
    def output_morph_dict(self):
        ...
        argsv=list(alias_cleaned.keys())
        argsv.sort()
        cats=[]

        for output in argsv:
            domain_arr=alias_cleaned[output]
            domain_arr=';'.join([arg_cleaned[0]+','+'|'.join(arg_cleaned[1]) for arg_cleaned in domain_arr])
    ...
    def load_xtag_morph(self):
        ...
        while info_dictv:
            ...
            dirnames=[arg_cleaned.split() for arg_cleaned in dirnames]
