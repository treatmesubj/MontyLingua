...
from MontyLingua import MontyTokenizer, MontyTagger, MontyLemmatiser, MontyREChunker, MontyExtractor, MontyNLGenerator
...
#
    ...
    each sentence
        ...
        tagged = list(map(self.tag_tokenized,tokenized))
        chunked = list(map(self.chunk_tagged,tagged))
        #print "CHUNKED: " + string.join(chunked,'\n         ')
        extracted = list(map(self.extract_info,chunked))
        return extracted
    ...
    def pp_info(self,extracted_infos):
        ...
        for i in range(len(extracted_infos)):
            keys = list(extracted_infos[i].keys())
            keys.sort()
            print("\n\n   SENTENCE #%s DIGEST:\n"%str(i+1))
            for key in keys:
    ...
    def strip_tags(self,tagged_or_chunked_text):
        ...
        toks = [x for x in toks if '/' in x]
        toks = [x.split('/')[0] for x in toks]
        return ' '.join(toks)
